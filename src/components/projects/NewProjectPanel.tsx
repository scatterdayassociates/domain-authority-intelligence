      ...
      <div className={`fixed top-0 right-0 bottom-0 w-[440px] bg-white shadow-2xl z-50 transition-transform duration-200 ease-out ${open ? "translate-x-0" : "translate-x-full"}`}>
        ...
        <div className="px-6 overflow-y-auto" style={{ height: "calc(100% - 80px)" }}>
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            <div>
              <label className="text-xs text-slate-500 mb-1 block">Advertiser / Brand</label>
              <input className="w-full h-9 border border-slate-200 rounded-md px-3 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-teal-500" placeholder="e.g. Dell Technologies" />
            </div>
            <div>
              <label className="text-xs text-slate-500 mb-1 block">Vertical</label>
              <input className="w-full h-9 border border-slate-200 rounded-md px-3 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-teal-500" placeholder="e.g. Consumer Electronics" />
            </div>
            <div>
              <label className="text-xs text-slate-500 mb-1 block">Category</label>
              <input className="w-full h-9 border border-slate-200 rounded-md px-3 text-sm text-slate-800 focus:outline-none focus:ring-2 focus:ring-teal-500" placeholder="e.g. Laptops" />
            </div>
            <div>
